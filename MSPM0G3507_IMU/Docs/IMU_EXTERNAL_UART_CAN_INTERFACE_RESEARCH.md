...
本轮先做外部 MCU UART，不改变 Type-C/HTML `HostLink`。

- UART 外部接口使用 `UART_EXTERNAL`，引脚为 `PB15/PB16`，当前波特率设为 `460800`，8N1。
- 新增 `ExternalIMULink` 任务负责外部 MCU 通信。
- JY901 兼容模式只输出数据，不实现 JY901 寄存器读写、修改配置和保存参数命令。
- JY901 兼容输出模式下仍然解析我们的 Native `A5 5A` 命令帧，用于调试页触发零飘、AutoC、1080 标定和角度归零。
- JY901 兼容输出按 WIT/JY901 常见默认回传频率 `10 Hz` 推送，每次输出一帧 `0x55 0x52` 角速度和一帧 `0x55 0x53` 角度。
- 自定义协议使用 `A5 5A` 帧头，包含 command、response、data 三类帧，使用 CRC16-CCITT。
- 自定义协议默认 `100 Hz` 发送轻量数据/状态帧。
- 自定义数据帧不发送陀螺仪原始 raw，也不发送 bias、温度、scale 等内部诊断参数；只发送已解算/校正后的角速度、角度、归一化角度、状态和必要 flags。
- 自定义数据帧 payload 固定 `20 bytes`，完整帧固定 `27 bytes`。`flags bit0` 表示正在静止自学习 bias，`flags bit1` 表示外部 UART RX 发生过溢出，`flags bit2` 表示角度数据有效。
- 本次启动校准成功前，Native 状态帧继续发送，但角速度和角度字段填 0；JY901 兼容流没有状态位，角度有效前不发送 JY901 数据帧。
- 本轮不实现运行时修改发送频率的命令；后续需要时再单独增加。
