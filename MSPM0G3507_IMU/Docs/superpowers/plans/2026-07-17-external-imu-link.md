...
JY901 兼容模式不接收命令，也不承诺可以被 JY901 工具完整配置。它的目的只是让已有 WIT/JY901 风格解析器能看到基本角速度和角度。
...
typedef enum {
    ...
    EXTERNAL_IMU_CMD_SET_STREAM_MODE = 0x20,
...
SET_STREAM_MODE -> 参数合法则更新 RAM 中的输出模式，回复 OK；非法则回复 BAD_PARAM
未知命令   -> 回复 BAD_COMMAND
```
...
#define EXTERNAL_IMU_LINK_JY901_PERIOD_MS  (10U)
```

- [ ] JY901 兼容流启用时，交替发送角速度帧和角度帧。
...
```text
能看到 55 52 和 55 53 JY901 兼容帧，SUM 正确。
